...
const init = () => {
    ...
    port = browser.runtime.connect({name: "extension_socket"});
    ...
    browser.storage.local.get(['websites'], function(result) {
        if(!isEmpty(result)){
    ...
    browser.storage.local.get(['sources'], function(result){
        documentUrlData = result.sources;
    })

    //copy/paste
    document.getElementById("copy-data-button").addEventListener('click', (event) => {
        
        copyTextToClipboard(JSON.stringify(buildCopyData(websiteData,companyData,documentUrlData)));
    });
...
const onMessage = data => {
    if(data.type=="packetIn"){
        ...
        if(data.frame ===0 && data.documentUrl !== undefined){
            if(documentUrlBool) documentUrlData = buildInitiatorData(data,documentUrlData);
            if(!websiteData.hasOwnProperty(data.documentUrl)){
            ...
            } else if(!websiteData[data.documentUrl].hasOwnProperty(data.company)){
                let builtwebsitedataPromise = buildWebsiteData(websiteData,data)
                builtwebsitedataPromise
                .then(
                    globalData => webPercentData = reduceWebsites(globalData)
...
const buildWebsiteData = (globalData, inData) =>{
    return new Promise ((webDataChange) => {
        assign(globalData, [inData.documentUrl, inData.company],  1); // update the website global variable websiteData[data.documentUrl][data.ip.company] ||
...
const clearHistory = () => {
    if (window.confirm("Are you sure you want delete your history? This cannot be undone.")) {
        ...
        documentUrlData = {};
        browser.storage.local.set({sources: documentUrlData}, function() {
            // console.log(sources);
        });

        browser.runtime.sendMessage({message: "clearing history"});
...
const buildCopyData = (websiteData, companyData, documentUrlData) => {
    let websiteDataTrue = convertTrue(websiteData)
    let copyData;
    if(documentUrlBool){
        copyData = {totalPacketCounts: companyData, websites: websiteDataTrue, sources: documentUrlData};
    } else {
        copyData = {totalPacketCounts: companyData, websites: websiteDataTrue};
    }
    return copyData
}
const buildInitiatorData = (inData,documentUrlData) => {
    if(documentUrlData[inData.documentUrl]){
        let size = Object.keys(documentUrlData[inData.documentUrl]).length;
        let name = "Packet " + size;
        documentUrlData[inData.documentUrl][name] = {company: inData.company, url: inData.url, ip: inData.ip};
        browser.storage.local.set({sources: documentUrlData}, function() {
            // console.log(documentUrlData);
        });
        return documentUrlData
    } else{
        documentUrlData[inData.documentUrl] = {
